...
function Text({ children, size = 1, left, right, top, bottom, color = "white", opacity = 1, height = 0.01, layers = 0, font = "/MOONGET_Heavy.blob", ...props }: any) {
  const data: any = useLoader(FontLoader, font)
  const geom: any = useAsset(() => new Promise((res) => res(new TextBufferGeometry(children, { font: data, size: 1, height, curveSegments: 32 }))), [children])
...
const MultilineText = ({ text, size = 1, lineHeight = 1, position = [0, 0, 0], ...props }: any) =>
  text.split("\n").map((text: any, index: number) => <Text key={index} size={size} {...props} position={[position[0], position[1] - index * lineHeight, position[2]]} children={text} />)
